	...
	for (; itr != startPoints.end(); ++itr) {
		...
		while (step < maxStrokeWidth_) {
			...
			if (edgemap_.at<unsigned char>(currY, currX) == 255) {
				...
				if (abs(abs(iTheta - jTheta) - 3.14) < 3.14 / 6) {
					isStroke = true;
					if (purpose == UPDATE) {
...
// Author: Luiz Paulo Rabachini
void DetectText::createImgBW(const char* pathToImgBW) {
	Mat dst;

//	img = img + Scalar(50, 50, 50);
//	threshold(img, dst, 128, 255,  CV_THRESH_BINARY | CV_THRESH_OTSU);
//	adaptiveThreshold(img, dst, 255, ADAPTIVE_THRESH_GAUSSIAN_C, CV_THRESH_BINARY, 99, -30);

	medianBlur(image_, dst, 3);

	imwrite(pathToImgBW, dst);
}
